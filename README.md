...
After duplicating this repository, you may rename the default package name, which is `naat-bdd`, with your package name wherever it is declared.
So rename it in:
* `pyproject.toml`
* `tests.yaml`
* `launch.json`
...
```shell
pip install --index-url https://test.pypi.org/simple/ naat-bdd
```
or
```shell
pip3 install --index-url https://test.pypi.org/simple/ naat-bdd
```
